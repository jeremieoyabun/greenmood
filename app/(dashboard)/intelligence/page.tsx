...
export default async function IntelligencePage() {
  ...
  return (
    <>
            ...
            return (
              <Card key={signal.id} className={i === 0 ? 'ring-1 ring-gm-sage/20' : ''}>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs text-gm-cream/30 ml-auto">Score: {signal.score}</span>
                </div>
                <h3 className="text-sm font-semibold text-gm-cream mb-1 line-clamp-2">{signal.title}</h3>
                <p className="text-xs text-gm-cream/40 line-clamp-2 mb-2">{signal.whyItMatters}</p>
                <div className="flex items-center gap-1">
                  {signal.recommendedFormat && (
                    <span className="text-xs px-1.5 py-0.5 bg-gm-sage/10 rounded text-gm-sage/60">{signal.recommendedFormat}</span>
                  )}
                  {signal.recommendedChannel && (
                    <span className="text-xs px-1.5 py-0.5 bg-white/[0.04] rounded text-gm-cream/30">{signal.recommendedChannel}</span>
                  )}
                </div>
              </Card>
            )
      ...
      {/* Competitors */}
      <div className="mb-6">
        <h2 className="text-xs uppercase tracking-wider text-gm-cream/40 font-semibold mb-3">Competitor Watchlist</h2>
          ...
          {competitors.map((comp) => {
            ...
            return (
              <Card key={comp.id} hover>
                ...
                <p className="text-xs text-gm-cream/30 mb-1">{comp.country}</p>
                <p className="text-xs text-gm-cream/25 line-clamp-1">{comp.positioning}</p>
                <a
                  href={comp.website}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-xs text-gm-sage/40 hover:text-gm-sage mt-1 block truncate"
                >
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-xs uppercase tracking-wider text-gm-cream/40 font-semibold">Signal Feed</h2>
          <span className="text-xs text-gm-cream/25">{signals.length} signals — {highPriority.length} high priority</span>
        </div>
                  ...
                  <div className="flex gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        ...
                        {signal.competitor && (
                          <span className="text-xs text-gm-cream/40">{signal.competitor.name}</span>
                        )}
                        {signal.source && (
                      ...
                      <h3 className="text-sm font-semibold text-gm-cream/90 mb-1">{signal.title}</h3>
                      <p className="text-xs text-gm-cream/50 mb-2 leading-relaxed">{signal.summary}</p>

                      {/* Why it matters */}
                      <div className="bg-gm-sage/5 rounded p-2 mb-2 border border-gm-sage/10">
                        <p className="text-xs text-gm-sage/70 leading-relaxed">
                      ...
                      <div className="bg-white/[0.02] rounded p-2 border border-white/[0.04]">
                        <p className="text-xs text-gm-cream/40 leading-relaxed">
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        ...
                        {signal.recommendedChannel && (
                          <span className="text-xs px-1.5 py-0.5 bg-sky-500/10 rounded text-sky-400/60">{signal.recommendedChannel}</span>
                        )}
                        <span className="text-xs text-gm-cream/15 ml-auto">
      ...
      {/* Sources */}
      <div>
        <h2 className="text-xs uppercase tracking-wider text-gm-cream/40 font-semibold mb-3">Active Sources ({sources.length})</h2>
        <div className="grid grid-cols-3 gap-2">
          {sources.map((source) => (
            <Card key={source.id}>
              <div className="flex items-center gap-2 mb-1">
                <h3 className="text-xs font-medium text-gm-cream/70">{source.name}</h3>
                <Badge variant="default" size="sm">{source.type}</Badge>
              </div>
              <div className="flex flex-wrap gap-1">
                {source.topics?.slice(0, 3).map((t: string) => (
                  <span key={t} className="text-[11px] px-1 py-0.5 bg-white/[0.03] rounded text-gm-cream/20">{t}</span>
                ))}
              </div>
              {source.url && (
                <a href={source.url} target="_blank" rel="noopener noreferrer" className="text-xs text-gm-sage/30 hover:text-gm-sage mt-1 block truncate">
